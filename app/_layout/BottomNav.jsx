...
import { Home, ShoppingBag, ShoppingCart, Package, Heart, User, X, Store, LayoutDashboard } from 'lucide-react'
import { useCartStore, useAuthStore } from '../../src/lib/store'
    ...
    <nav className="bottom-nav">
      {tabs.map(tab => (
        <button key={tab.label}
          onClick={() => tab.action ? tab.action() : router.push(tab.to)}
          className={`bn-item ${tab.to && pathname === tab.to ? 'active' : ''}`}>
          {tab.icon}
...
export function SideMenu({ open, onClose }) {
  ...
  const go = (to) => { router.push(to); onClose() }
